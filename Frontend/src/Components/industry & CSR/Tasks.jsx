...
const Tasks = ({ reports = [], loading = false, onChanged, orgName, onOffered }) => {
  ...
  async function handleFund(id) {
    ...
    try {
      await api.updateReport(id, {
        assignedIndustry: orgName || 'CSR Partner',
        status: 'collaboration',
        note: `${orgName || 'Industry partner'} committed CSR support and joined as a collaborator`,
      })
      toast('Support offered — check "Active Collaborations" to propose a solution!')
      onChanged?.()
      onOffered?.()
    } catch (err) {
      toast(err?.message || 'Could not offer support on this challenge', { type: 'error' })
    } finally {
      setFundingId(null)
    }
  }
  ...
  <div className="mt-4 flex gap-3">
    ...
    <select
      ...
      className="h-11 w-44 appearance-none rounded-2xl border border-slate-200 bg-slate-50 px-4 text-sm font-medium text-slate-800 outline-none"
    >
...
{loading ? (
  ...
  <div className="w-full min-h-96.25 rounded-[30px] border border-slate-200 bg-white p-6 shadow-sm mt-5 flex items-center justify-center text-slate-400 text-center px-6">
    No challenges currently open for CSR funding.
  </div>
) : (
  ...
  <div className="mb-3 flex flex-wrap items-center gap-4 text-xs text-slate-400">
    ...
    <span className="flex items-center gap-1">
      📅 {new Date(r.createdAt).toLocaleDateString()}
          ...
          {/* Button */}
          <button
            onClick={() => handleFund(r.id)}
            disabled={fundingId === r.id}
            className="mt-6 flex h-15 w-full items-center justify-center gap-2 rounded-[20px] bg-amber-500 text-base font-bold text-white shadow-[0_6px_12px_rgba(217,119,6,0.25)] transition hover:bg-amber-600 disabled:opacity-60"
          >
            <span>🤝</span>
            <span>{fundingId === r.id ? 'Offering Support…' : 'Offer Support & Collaborate'}</span>
          </button>
